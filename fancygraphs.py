        ...
        if constcolour:
            ...
            for i in range(-gridint, gridint+1):
                for j in range(-gridint, gridint+1):
                    ...
                    if f == [0,0]:
                        self.svg.draw_circ(self.tranx(x), self.trany(y), scale * 70, fill=vec_to_hex(endColour), strokewidth=0)
                    else:
...
A.VectorField(func, gridint=25,  arrow_scale=1.5, stroke_width=2, length_scale=0.5, grids=[30,30], arrow = True, constcolour=False, bg=True, constLength=True, initColour=hex_to_vec('#000099'), endColour=hex_to_vec('#ff99cc'), grid=False)
